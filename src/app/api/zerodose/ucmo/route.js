  ...
  try {
    ...
    if (user.designation !== "ucmo") {
      return NextResponse.json(
        {
          success: false,
          message: "Only ucmos can access this data.",
        },
        { status: 403 },
      );
    }
    ...
    /*
     * Base match:
     * Only selected campaign + authenticated ucmo's
     * Union Council.
     */
    const baseMatch = {
    ...
    console.error("ucmo Zerodose Error:", error);

    return NextResponse.json(
      {
        success: false,
        message: "Failed to get ucmo Zerodose data.",
      },
      { status: 500 },
    );
  }
