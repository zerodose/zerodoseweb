...
export default function VaccinatorZerodoseForm({ zerodoseId = null }) {
  ...
  const handleCover = useCallback(async () => {
    ...
    if (!qrCode || !/^\d{14}$/.test(qrCode)) {
      toast.error("QR code must be exactly 14 digits.");
      return;
    }
    ...
    try {
      ...
      await vaccinatorCover(zerodoseId, qrCode);
  ...
  }, [zerodoseId, qrCode, zerodose?.vaccinationStatus, router]);
  // ============================================================
  // LOADING
  // ============================================================
  ...
  return (
    <div className="min-h-full">
      ...
      <div className="space-y-5">
        ...
        {showVisit && !isCovered && (
          <section className="border-border bg-background overflow-visible rounded-2xl border shadow-sm">
      ...
      {showScanner && (
        <QrScannerModal
          ...
          onScan={(scannedValue) => {
            if (!/^\d{14}$/.test(scannedValue)) {
              toast.error("Scanned QR code must contain exactly 14 digits.");
              return;
            }
      ...
      {showCover && !showScanner && !isCovered && (
        <CoverQrModal
          qrCode={qrCode}
          setQrCode={setQrCode}
          onClose={() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-background w-full max-w-md overflow-hidden rounded-2xl shadow-2xl">
        ...
        <div className="border-border flex items-center justify-between border-b p-4">
          <div>
            <h3 className="text-text font-semibold">Cover Child</h3>

            <p className="text-text-secondary mt-1 text-xs">
              Enter or scan the 14-digit QR code.
            </p>
          </div>
        ...
        <div className="p-5">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder="Enter 14-digit QR code"
              maxLength={14}
              disabled={loading}
              className="border-border bg-background text-text placeholder:text-text-secondary focus:border-primary focus:ring-primary/20 h-11 min-w-0 flex-1 rounded-lg border px-3 text-sm outline-none focus:ring-2 disabled:cursor-not-allowed disabled:opacity-50"
            />
            ...
            <button
              ...
              disabled={loading}
              title="Scan QR code"
              className="bg-primary/10 text-primary hover:bg-primary/20 flex h-11 w-11 shrink-0 items-center justify-center rounded-lg transition disabled:cursor-not-allowed disabled:opacity-50"
            >
              <QrCode size={20} />
            </button>
          </div>
          ...
          <div className="mt-2 flex items-center justify-between">
            <p className="text-text-secondary text-xs">
              QR code must contain exactly 14 digits.
            </p>

            <span
              className={`text-xs font-medium ${
                qrCode.length === 14 ? "text-green-600" : "text-text-secondary"
              }`}
            >
              {qrCode.length}/14
            </span>
          ...
          <div className="mt-6 flex gap-3">
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="border-border text-text hover:bg-surface flex h-11 flex-1 items-center justify-center rounded-lg border px-4 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-50"
            >
              Cancel
            </button>
            ...
            <button
              type="button"
              onClick={onSave}
              disabled={loading || qrCode.length !== 14}
              className="bg-primary hover:bg-primary-dark flex h-11 flex-1 items-center justify-center gap-2 rounded-lg px-4 text-sm font-medium text-white transition disabled:cursor-not-allowed disabled:opacity-50"
            >
              <CheckCircle2 size={17} />
              Save
