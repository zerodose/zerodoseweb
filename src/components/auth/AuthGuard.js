...
export default function AuthGuard({ children }) {
  ...
  useEffect(() => {
    ...
    if (publicRoutes.includes(pathname)) {
      ...
      // Agar user already login hai aur auth page par aa gaya hai
      // to usay apne dashboard par wapas bhej do.
      // ----------------------------------------------------------
        ...
        try {
          ...
          if (
            ...
            if (allowedBaseRoute) {
              ...
              // Current history entry ko dashboard se replace karo
              // taake Back button se login page par wapas na aaye.
              router.replace(allowedBaseRoute);
