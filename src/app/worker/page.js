...
import { getZerodoseforWorker } from "@/api/zerodoseApi";
import { getCampaigns } from "@/api/campaignApi";
...
export default function Page() {
  ...
  const fetchWorkerData = useCallback(async () => {
    ...
    const normalizedSummary = {
      ...
      recorded: Number(workerSummary?.recorded || 0),
      visited: Number(workerSummary?.visited || 0),
      covered: Number(workerSummary?.covered || 0),
    };
    ...
    const zerodoseResponse = await getZerodoseforWorker({
  ...
  const loadMoreZerodose = async () => {
    ...
    try {
      ...
      const response = await getZerodoseforWorker({
