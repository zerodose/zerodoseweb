...
return (
  uniqueCampaigns.find(
    (campaign) =>
      String(getId(campaign)) === String(selectedCampaignId),
  ) || null
);
...
return data.filter((item) => {
  ...
  return (
    campaignId &&
    String(campaignId) === String(selectedCampaignId)
  );
});
...
// Do not automatically select month/campaign.
setSelectedMonth("");
setSelectedCampaignId("");
...
// Do not automatically select campaign.
setSelectedCampaignId("");
...
// Fetch Zerodose data only after campaign selection.
if (onCampaignSelect) {
  onCampaignSelect(value, "recorded");
}
...
====================================================== */}
  ...
  <div className="mb-5">
    <div className="flex items-center gap-2">
      ...
      <div>
        ...
        <p className="text-text-secondary mt-0.5 text-xs md:text-sm">
          Select a campaign to view team-wise and individual
          Zerodose records.
        </p>
      </div>
    </div>
  ...
  <CampaignFilter
    filter={{
      ...
      monthOptions: months.map((month) => ({
        value: String(month),
        label: String(month),
      })),

      campaignSelectOptions: campaignOptions
        .map((campaign) => {
          const campaignId = getId(campaign);
