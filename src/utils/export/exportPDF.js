...
export default function exportPDF({
  ...
  // Columns that should actually be formatted as dates
  dateColumns = [],
}) {
  if (!data.length) {
    return;
  ...
  const formatValue = (value, column) => {
    ...
    //
    // IMPORTANT:
    // Only columns explicitly present in dateColumns
    ...
    // Everything Else
    //
    // Do NOT use Date.parse() here.
    // Addresses, phone numbers, team numbers, codes, etc.
    // must remain normal text.
    // ==========================================================
  ...
  const rows = data.map((row, index) => [
    index + 1,
    ...columns.map((column) => formatValue(row[column], column)),
  ]);
