  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      try {
        ...
        try {
          const response = await getTownSummary();

          const data = response?.data || {};

          console.log("townfp data==>", data);

          if (!cancelled) {
            setCounts((previous) => ({
              ...previous,

              unionCouncils: data.unionCouncilCount ?? 0,
              ucmos: data.ucmoCount ?? 0,
              supervisors: data.supervisorCount ?? 0,
              teams: data.teamCount ?? 0,

              recorded: data.recordedCount ?? 0,
              visited: data.visitedCount ?? 0,
              covered: data.coveredCount ?? 0,
            }));
          }
        } catch (error) {
          console.error("Failed to fetch Town FP summary:", error);
        }
      } catch (error) {
        console.error("Failed to load Town FP dashboard:", error);
      } finally {
        if (!cancelled) {
          setLoading(false);
        }
